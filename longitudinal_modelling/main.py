...
df['age_at_score'] = df['age_at_score'].clip(lower=10)
df['readmission'] = 1 * (df['num_ward_entries'] > 1)
df['ward_len_normalized'] = df['ward_len'] / df['ward_len'].max()
df['nlp_sc_bool'] = np.where(df['nlp_num_symptoms'] >= 1, 1, 0)
...
res = fit_mlm(df, group=obj.group, target=obj.target, covariates=['age_at_score']+cov_sociodem+cov_meds_baseline, timestamp=obj.timestamp, rdn_slope=True, method=['lbfgs'])
...
res = fit_mlm(df, group='brcid', target='num_symptoms', covariates=cov_sociodem, timestamp=obj.timestamp, rdn_slope=True, method=['lbfgs'])
