...
df_nlp_ci = pd.read_excel(xls, 'nlp_ci')[0:100]
df_docs = pd.read_excel(xls, 'docs')[0:100]
df_honos = pd.read_excel(xls, 'honos')[0:100]
...
df.at[0, 'baseline'] = 1
del df_pop, df_nlp_ci, df_honos, df_ward, df_meds
for col in [col for col in df.columns if '_tomerge' in col]:
    df[col.replace('_tomerge','')].fillna(df[col], inplace=True)

## ADD NEW VARIABLES
df['diagnosis'] = np.where(df.has_dementia == 'dementia', 'sz_dementia', 'sz_only')
...
# TODO
df['age_at_score'] = np.minimum(10, df.year - df.dob.dt.year)
df['score_year_centered'] = 1 + np.where(df['baseline'] == 1, 0, df.year.diff())

###############################################
# TRAJECTORY ANALYSIS
###############################################
df_traj_honos = df.dropna(subset=cov_honos)
baseline_cols=['brcid', 'antidementia', 'antipsychotics', 'antidepressant']
df_baseline = df_traj_honos.loc[df.score_year_centered == 1][baseline_cols]
